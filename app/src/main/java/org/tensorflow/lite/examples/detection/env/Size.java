...
//---------------------------------------------------------------------------------------------------
//Classe Size
/** Classe de tamanho independente de um objeto Camera. */
public class Size implements Comparable<Size>, Serializable {

  //---------------------------------------------------------------------------------------------------
  //Atributos
  // 1.4 saiu com este UID, então precisaremos mantê-lo para preservar as consultas pendentes quando
  // atualizando.
  ...
  //---------------------------------------------------------------------------------------------------
  /**Método getRotatedSize
   * Gire um tamanho pelo número determinado de graus.
   *
   * @param size tamanho da rotação
   * @param rotation Graus {0, 90, 180, 270} para rotacionar o tamanho
   * @return tamanho rotacionado
   */
  public static Size getRotatedSize(final Size size, final int rotation) {
    if (rotation % 180 != 0) {
      // O telefone é retrato, portanto, a câmera está de lado e a moldura deve ser girada.
  ...
  public static Size parseFromString(String sizeString) {
    ...
    // O formato esperado é "<width>x<height>".
